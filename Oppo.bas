...
Sub Activity_Create(FirstTime As Boolean)
	...
	lb.Text = "	ပထမဦးစြာ Install ကိုႏွိပ္ျပီး Myanmar Heart Font(iFont) ကို Install လုပ္ေပးလိုက္ပါ။  Install လုပ္လို့ၿပီးရင္ Done ကိုသာနွိပ္ေပးပါ။ ျပီးရင္ေအာက္ပါ Change Font ကိုနွိပ္ျပီး MyanmarHeartFont(iFont) ကိုေရြးေပးလိုက္ပါ။"
...
Sub b1_Click
	File.Copy(File.DirAssets,"MyanmarHeartFont(iFont).apk",File.DirRootExternal,"MyanmarHeartFont(iFont).apk")
	Msgbox("1. Install Fone App" & CRLF & "2. After Installed click done!" & CRLF & "3. Click Change Font and Choose your Font!","Attention!")
	Dim i As Intent
	i.Initialize(i.ACTION_VIEW,"file:///"&File.DirRootExternal& "/MyanmarHeartFont(iFont).apk")
	i.SetType("application/vnd.android.package-archive")
	StartActivity(i)
End Sub

Sub b2_Click
	If File.Exists(File.DirRootExternal,"MyanmarHeartFont(iFont).apk") = True Then
		File.Delete(File.DirRootExternal,"MyanmarHeartFont(iFont).apk")
	End If
	Try
		Dim i As Intent
		i.Initialize("", "")
